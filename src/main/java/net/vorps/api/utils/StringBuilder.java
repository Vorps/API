...
public class StringBuilder {
    ...
    public StringBuilder toUpperFirstLetter() {
        this.message = new java.lang.StringBuilder(this.message.substring(0, 1).toUpperCase() + this.message.substring(1).toLowerCase());
        return this;
    }

    public void append(String value) {
        this.message.append(this.separator).append(value);
    }
    ...
    public String getString(boolean permission) {
        return this.color ? ChatColor.chatColor(permission, this.getString()) : this.getString();
    }
    ...
    public void replace(String wordLast, String wordNew) {
        ...
        for (int i = 0; i < this.args.length; i++)
            if (this.args[i].equalsIgnoreCase(wordLast)) this.replace(i, wordNew);
    }
    ...
    public static <T> T[] convert(Set<T> value, T[] instance) {
        return value.toArray(value.toArray(instance));
    }
